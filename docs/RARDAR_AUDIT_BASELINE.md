...
| P1-3 行动事件 | 由当前行动事件分支实现；仅在对应 PR 合并到 main 后视为完成 | 合并前不得开始下一工程目标 |
| P1-4 评分语义 | 当前行动事件 PR 合并后的下一工程目标 | 合并后按 `CODEX_MASTER_INSTRUCTION.md` 开始独立工程轮 |
| P1-5 verify/CI | 待处理 | 尚未开始独立工程轮 |
| P1-6 稳定项目 ID | 待处理 | 尚未开始独立工程轮 |

当前行动事件 PR 合并前不得开始 P1-4 评分语义或其他后续工程目标。
