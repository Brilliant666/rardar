...
- legacy slug 无匹配、多匹配，projectId 不存在，或两个 selector 不一致时均 fail closed；
...
正式 `0004` 不读取 generation 文件，也不把 slug 猜成 repository。它只建立 additive 结构；需要事实映射的 preflight、backfill 和 active switch 由持有 verified Catalog map 的运行时 adoption 完成，而不是由第二份 DDL 或数据库自行推断。任一 unresolved 或非法 legacy 行会使整次 adoption fail closed，不静默跳过。
