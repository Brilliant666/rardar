...
11. flat staging 可用 `python -m pipeline.migrate_project_identity --data-dir data` 预检和 dry-run，再显式加 `--apply` 迁移可信 v1 artifact；无法机械升级的 legacy v0 只报告并保留。工具只处理 `data/analysis` 与 `data/enrichment`，不跟随符号链接、不访问 retained generations、不修改 current、不发布 generation；等价目标不重写，apply 可在中断后继续清理遗留源，正常完成后的重复 apply 为 no-op；冲突或无法唯一归属的旧 slug 列为 unresolved 并保留原文件。
