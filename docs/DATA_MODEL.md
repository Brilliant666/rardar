...
legacy adoption 只通过明确 Catalog 映射进行。`project_actions`、`project_action_events`、`project_action_state`、`feedback` 和 `decision_events` 的原始 action/value、`occurred_at`/`created_at` 与真实阶段原样保留；确定性 legacy 幂等键继续复用，不从 State 补造 Event，也不推断 owner 转移、仓库改名或缺失的历史。任一 slug 无匹配或多匹配、映射冲突、非法行动或无法解析的时间都会返回稳定错误并阻止 `project_identity_runtime` 切换，不能静默跳过后声称迁移完成。
