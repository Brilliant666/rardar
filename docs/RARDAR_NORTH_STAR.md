...
近 7 天发生以下任一行动、并以 Stable Project ID 识别的不同项目数：
...
- Event 表记录发生过什么。
- State 表记录现在是什么状态。
- canonical Event 与 State 使用版本化 Stable Project ID，不能继续把可碰撞的 slug 当作唯一身份。
...
- Node dependency resolution、完整 Verify、build 与 Python wheel preparation 必须在隔离 CI 中完成并绑定成功 Verify 的 exact SHA；Production 只允许校验、解包、离线 venv、preflight 与原子激活，不访问 npm registry、不运行 npm install/build；
