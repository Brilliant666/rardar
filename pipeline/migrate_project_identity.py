"""Safely migrate flat project artifacts from legacy slugs to stable IDs.
...
def _safe_target_path(directory: Path, project_id: str) -> Path:
    if (
        not project_id
        or project_id in {".", ".."}
        or "/" in project_id
        ...
        raise ProjectIdentityMigrationError(
            "unsafe_project_id_path",
            f"stable project ID is not a safe filename segment: {project_id!r}",
        )
    ...
    try:
        ...
        raise ProjectIdentityMigrationError(
            "staging_path_escape",
            f"stable project target cannot be resolved safely: {target}: {error}",
        ) from None
    if not _same_path(resolved_parent, directory):
        raise ProjectIdentityMigrationError(
            "staging_path_escape",
            f"stable project target escapes its staging directory: {target}",
        )
    return target
...
def _preflight(data_dir: Path) -> _Preflight:
    ...
    for artifact in artifacts:
        if artifact.version not in {0, 1}:
            continue
        owners = legacy_owners.setdefault(artifact.path.stem, {})
    ...
    for artifact in artifacts:
        ...
        try:
            validated_expected = require_valid(
                artifact.kind,
                expected_payload,
                source_path=target,
            ...
            raise ProjectIdentityMigrationError(
                ...
                f"mechanically migrated artifact fails the current contract: {target}: {error}",
            ) from None
        ...
        if target_artifact is not None and target_artifact.payload != validated_expected:
            raise ProjectIdentityMigrationError(
                "target_conflict",
                f"stable target already exists with different content or ownership: {target}",
            )
        if target_artifact is None and os.path.lexists(target):
            ...
            raise ProjectIdentityMigrationError(
                "unsafe_target",
                f"stable target exists but was not a safe preflight artifact: {target}",
            )
        actions.append(
            _MigrationAction(
                source=artifact,
    ...
    return _Preflight(tuple(artifacts), tuple(actions))
...
def _verify_source_unchanged(action: _MigrationAction) -> None:
    ...
    try:
        ...
        raise ProjectIdentityMigrationError(
            ...
            f"legacy source became unavailable during apply: {action.source.path}: {error}",
        ) from None
    if current != action.source.source_bytes:
        raise ProjectIdentityMigrationError(
            "source_changed_during_apply",
            f"legacy source changed after preflight: {action.source.path}",
        )
...
def _verify_target(action: _MigrationAction) -> None:
    ...
    try:
        ...
        raise ProjectIdentityMigrationError(
            ...
            f"stable target failed post-write verification: {action.target}: {error}",
        ) from None
    if validated != action.expected_payload:
        raise ProjectIdentityMigrationError(
            "target_changed_during_apply",
            f"stable target differs from the preflight payload: {action.target}",
        )
...
def _verify_apply_preconditions(preflight: _Preflight) -> None:
    ...
    for action in preflight.actions:
        ...
        if action.target_already_equivalent:
            ...
            raise ProjectIdentityMigrationError(
                "target_changed_during_apply",
                f"stable target appeared after preflight: {action.target}",
            )
...
def _report(preflight: _Preflight, data_dir: Path, *, apply: bool) -> dict[str, Any]:
    ...
    for artifact in preflight.artifacts:
        ...
        if artifact.version == 0:
            ...
            status = "equivalent_target_cleaned" if apply else "would_cleanup_equivalent_target"
    ...
    return {
        ...
        "alreadyCurrentCount": sum(
            artifact.version == 2 for artifact in preflight.artifacts
        ),
        "items": items,
    }


def migrate_project_identity(data_dir: Path, *, apply: bool = False) -> dict[str, Any]:
    ...
    with data_dir_lock(canonical):
        preflight = _preflight(canonical)
        if not apply:
            return _report(preflight, canonical, apply=False)

        # Complete every atomic target write before deleting any legacy source.
        ...
        for action in preflight.actions:
            ...
            try:
                ...
                raise ProjectIdentityMigrationError(
                    "target_write_failed",
                    f"stable target could not be written atomically: {action.target}: {error}",
                ) from None
        ...
        for action in preflight.actions:
            ...
            try:
                ...
                raise ProjectIdentityMigrationError(
                    ...
                    f"stable target is durable but legacy source cleanup was interrupted: "
...
def _argument_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Migrate flat Rardar project artifacts to stable project IDs"
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        report = migrate_project_identity(arguments.data_dir, apply=arguments.apply)
