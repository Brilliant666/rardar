...
评分语义 PR #6 已合并，最新 `main` 已包含 evidence-v2 评分与旧 generation 兼容。当前唯一工程目标是：
...
本 PR 已交付单一 `npm run verify`、数据与 Runtime 隔离保护、Pull Request/main push GitHub Actions，以及直接相关文档；只有对应 PR 合并到 `main` 后，P1-5 才最终视为完成。下一工程目标只记录为“稳定项目 ID”，本轮不得实现或提前开始。
...
5. verify 和 GitHub Actions——由当前 PR 交付，仅在对应 PR 合并到 `main` 后视为完成；
