...
| --- | --- |
| Lint | `npm run lint` |
| Python tests | `python -m unittest discover -s pipeline -p "test_*.py"` |
| Schema validation | `npm run data:validate` |
...
GitHub Actions 在指向 `main` 的 Pull Request 和 push 到 `main` 时，于 Ubuntu 上使用 Node.js 22.13.1、Python 3.12、`npm ci` 与 `requirements.lock` 锁定安装后调用同一个 `npm run verify`。
