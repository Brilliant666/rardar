...
入口会从项目画像中的 `repository` 计算唯一目标文件名；不能把正式文件本身当作草稿，也不能用一种产物覆盖另一种正式路径。

兼容规则不会伪造历史事实：GitHub snapshot v1 保留既有 `schema_version` 字段和早期 history 形状；缺少可信 `analyzed_at` 的两份旧静态证据显式标记为 `schemaVersion: 0`，当前分析输出使用 v1；signal enrichment v1 继续允许旧条目使用顶层 `generatedAt` 作为分析时间回退。详细模型见 `docs/DATA_MODEL.md`。
